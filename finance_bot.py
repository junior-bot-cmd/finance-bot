...
TIPS = [
    "💡 *Совет дня*\n\nПравило 50/30/20: 50% дохода — на нужное, 30% — на желания, "
    "20% — в накопления. Простая рамка, чтобы не уйти в минус.",

    "🧐 *А ты знал?*\n\n«Эффект латте»: кофе за 200 ₽ каждый день — это ~73 000 ₽ в год. "
    "Маленькие траты складываются в большие суммы.",

    "💡 *Совет дня*\n\nЗаписывай трату сразу, а не вечером. Через пару часов мы забываем "
    "до 60% мелких покупок — и бюджет «течёт».",

    "🛡 *Финансовая подушка*\n\nЭто 3–6 месяцев твоих расходов про запас. "
    "Начни хотя бы с суммы на один месяц — уже спокойнее жить.",

    "🧐 *Факт*\n\nСлово «salary» (зарплата) идёт от латинского *salarium* — "
    "деньги, которые римским солдатам выдавали на покупку соли.",

    "⏳ *Правило 24 часов*\n\nХочешь купить что-то не из плана — подожди сутки. "
    "Половина импульсивных желаний отпадает сама.",

    "🔁 *Проверь подписки*\n\nПодписки — тихий убийца бюджета. "
    "Часто мы платим за то, чем уже не пользуемся. Загляни в список списаний.",

    "📊 *Совет дня*\n\nВеди учёт хотя бы месяц — и удивишься, куда реально уходят деньги. "
    "Обычно это совсем не то, что кажется на первый взгляд.",

    "🎯 *Цель копит лучше*\n\nДеньги «просто отложить» тратятся легко. "
    "Дай накоплению имя — «отпуск», «новый телефон» — и копить станет проще.",

    "🧾 *Совет дня*\n\nКрупную покупку дели на «стоимость в днях работы». "
    "Вещь за 30 000 ₽ при доходе 90 000 ₽/мес — это 10 рабочих дней. Всё ещё хочется?",

    "🧐 *Факт*\n\nСредний человек принимает около *35 000* решений в день. "
    "Часть из них — про деньги. Поэтому простые правила экономят силы.",

    "💳 *Наличные vs карта*\n\nИсследования показывают: картой мы тратим легче, "
    "чем наличными — траты «не чувствуются». Записывай их, чтобы чувствовать.",
]
...
def _daily_message():
    ...
    if today.day == 1:
        return ("🗓 *Начало месяца!*\n\nСамое время запланировать бюджет на месяц "
                "и поставить лимиты по категориям. Загляни в раздел «🎯 Лимиты».")
    if tomorrow.day == 1:
        return ("🏁 *Конец месяца!*\n\nПодведи итоги: загляни в «📈 Отчёт», "
                "проверь подписки и regular-платежи. Что можно сократить в следующем месяце?")
    if today.day == 25:
...
async def _daily(app: Application):
    ...
    for u in users or []:
        try:
            await app.bot.send_message(u["user_id"], text, parse_mode="Markdown")
            await asyncio.sleep(0.05)
        except Exception as e:
            logger.error("daily reminder %s: %s", u["user_id"], e)
...
async def on_start(app: Application):
    ...
    sched.add_job(_daily,   CronTrigger(hour=15),                    args=[app])
    sched.start()
