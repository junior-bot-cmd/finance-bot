#!/usr/bin/env python3
# v5.0 — Finance Bot: Quick Add, History+Delete, Insights, Weekly Digest
...
# keyword → category (lower-case keys, checked with startswith/in)
...
def _db(query: str, params=(), fetch: str | None = None):
...
async def db(query: str, params=(), fetch: str | None = None):
...
def _bar_chart(daily: dict, title: str) -> io.BytesIO | None:
    if not daily:
        return None
...
# ── Expense flow (кнопка «Добавить расход») ───────────────────────────────────
...
# ── Quick add (e.g. «500 такси» or «1200 продукты кофе») ──────────────────────
...
async def quick_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if category:
        note_part = keyword[len(next(kw for kw in QUICK_MAP if keyword.startswith(kw) or kw in keyword)):].strip()
        ...
        await update.message.reply_text(
            f"✅ *{amount:,.2f} ₽* — {category}{alert}",
            parse_mode="Markdown",
            reply_markup=main_keyboard(),
        )
    else:
        # Category not recognised — save with "Другое" and use keyword as note
        await db(
            "INSERT INTO expenses (user_id, amount, category, note, expense_date) VALUES (%s,%s,%s,%s,%s)",
            (user_id, amount, "📦 Другое", m.group(2).strip(), date.today()),
        )
        await update.message.reply_text(
            f"✅ *{amount:,.2f} ₽* — 📦 Другое\n📝 {m.group(2).strip()}",
            parse_mode="Markdown",
...
async def stats_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    grand = sum(r["total"] for r in rows) if rows else 0
...
async def show_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Insights: avg daily, top weekday
    avg_row = await db(
    ...
    lm_total    = lm_row["total"] if lm_row else 0
    avg_daily   = avg_row["avg_day"] if avg_row else 0
    top_weekday = WEEKDAYS_RU[weekday_row["dow"]] if weekday_row else "—"
    ...
    lines.append(f"📆 Самый затратный день недели: *{top_weekday}*")
...
async def show_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        text_lines.append(f"{day} | {r['category']}{note}: *{r['amount']:,.0f} ₽*")
        buttons.append([InlineKeyboardButton(
            f"🗑 {day} {_strip_emoji(r['category'])} {r['amount']:,.0f}₽",
...
async def _send_monthly_reports(app: Application):
    ...
    for row in users or []:
        uid, name = row["user_id"], row["first_name"] or "друг"
        try:
            exp_rows = await db(
                "SELECT category, SUM(amount) AS total FROM expenses "
...
async def _send_weekly_digests(app: Application):
    ...
    for row in users or []:
        ...
        try:
            exp_rows = await db(
                "SELECT category, SUM(amount) AS total FROM expenses "
                "WHERE user_id=%s AND expense_date BETWEEN %s AND %s GROUP BY category ORDER BY total DESC",
                (uid, start_dt, today), fetch="all",
            )
            if not exp_rows:
                continue

            grand = sum(r["total"] for r in exp_rows)
            lines = [
            ...
            for r in exp_rows[:5]:
                pct = r["total"] / grand * 100 if grand > 0 else 0
...
async def _post_init(app: Application):
    ...
    # Monthly report: 1st of each month, 09:00
...
def main():
    ...
    # Quick add: number + space + text, only when not inside a conversation
